...
public abstract class NetworkSimulator {
	...
	private final double corruptProb;
	private final double avgMessageDelay;
	protected int traceLevel;
	private final EventList eventList;
	private FileWriter outFile;

	private final OSIRandom rand;
	...
	public void runSimulator() {
		...
		while (true) {
			...
			switch (next.getType()) {
			case TIMERINTERRUPT:
				if (next.getEntity() == NetworkSimulator.A)
					aTimerInterrupt();
				else
			...
			case FROMLAYER3:
				...
				else if (next.getEntity() == NetworkSimulator.B)
			...
			case FROMLAYER5:
				...
				final char[] nextMessage = new char[NetworkSimulator.MAXDATASIZE];

				// Now, let's generate the contents of this message
				final char j = (char) ((nSim % 26) + 97);
				for (int i = 0; i < NetworkSimulator.MAXDATASIZE; i++)
					nextMessage[i] = j;
	...
	private void generateNextArrival() {
		...
		final double x = 2 * avgMessageDelay * rand.nextDouble(0);
		final Event next = new Event(time + x, NetworkSimulator.FROMLAYER5,
				NetworkSimulator.A);
	...
	protected void stopTimer(int entity) {
		...
		final Event timer = eventList.removeTimer(entity);
	...
	protected void startTimer(int entity, double increment) {
		...
		final Event t = eventList.removeTimer(entity);
		...
		if (t != null) {
			...
			final Event timer = new Event(time + increment,
	...
	protected void toLayer3(int callingEntity, Packet p) {
		...
		int destination;
		double arrivalTime;
		final Packet packet = new Packet(p);

		if (traceLevel > 2)
			System.out.println("toLayer3: " + packet);

		// Set our destination
		if (callingEntity == NetworkSimulator.A)
			destination = NetworkSimulator.B;
		else if (callingEntity == NetworkSimulator.B)
		...
		if (rand.nextDouble(3) < corruptProb) {
			...
			final double x = rand.nextDouble(4);
			if (x < 0.75) {
		...
		final Event arrival = new Event(arrivalTime,
				NetworkSimulator.FROMLAYER3, destination, packet);
		eventList.add(arrival);
	}

	protected void toLayer5(String dataSent) {
		try {
			outFile.write(dataSent, 0, NetworkSimulator.MAXDATASIZE);
			outFile.write('\n');
		} catch (final Exception e) {
			e.printStackTrace();
		}
